...
const UNIDADES = ["Unidad", "Litro", "Mililitro", "Kilo", "Gramo", "Pack", "Caja", "Docena", "Metro", "Centímetro"];
...
function ProductModal({ product, onClose, onSaved }: { product: Product | null; onClose: () => void; onSaved: () => void }) {
  ...
  const initUnidad = String(product?.unidad_medida ?? "Unidad");
  const [unidadOtra, setUnidadOtra] = useState(() => initUnidad !== "" && !UNIDADES.includes(initUnidad));
  ...
  return <div className="modal-backdrop"><div className="modal"><div className="page-header"><div><h2 style={{ margin: 0 }}>{product ? "✏️ Editar producto" : "➕ Agregar producto"}</h2><p className="muted">Completa los datos del producto.</p></div><button className="btn btn-secondary" onClick={onClose}>Cerrar</button></div>
    <form onSubmit={save}>
      <div className="card section-card"><h3 className="section-title">📦 Identificación</h3><div className="form-grid">{field("nombre", "Nombre")}{field("codigo_barras", "Código de barras / SKU")}{field("marca", "Marca")}{field("categoria", "Categoría")}{field("sabor", "Sabor / Variante")}{/* Unidad de medida: select + "Otro" */}<div className="field"><label className="label">Unidad de medida</label><select className="select" value={unidadOtra ? "__otro__" : String(form.unidad_medida ?? "Unidad")} onChange={e => { if (e.target.value === "__otro__") { setUnidadOtra(true); set("unidad_medida", ""); } else { setUnidadOtra(false); set("unidad_medida", e.target.value); } }}>{UNIDADES.map(u => <option key={u} value={u}>{u}</option>)}<option value="__otro__">Otro…</option></select>{unidadOtra && <input className="input" type="text" value={String(form.unidad_medida ?? "")} onChange={e => set("unidad_medida", e.target.value)} placeholder="Escribí la unidad..." style={{ marginTop: 6 }} />}</div></div></div>
      ...
      <div className="card section-card"><h3 className="section-title">🗓️ Fechas</h3><div className="form-grid">{/* Fechas: date picker nativo */}<div className="field"><label className="label">Fecha de ingreso</label><input className="input" type="date" value={toInputDate(form.fecha_ingreso)} onChange={e => set("fecha_ingreso", e.target.value || "")} /></div><div className="field"><label className="label">Fecha de vencimiento</label><input className="input" type="date" value={toInputDate(form.fecha_vencimiento)} onChange={e => set("fecha_vencimiento", e.target.value || "")} /></div></div></div>
