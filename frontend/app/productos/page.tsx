...
function ProductModal({ product, onClose, onSaved }: { product: Product | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-backdrop"><div className="modal"><div className="page-header"><div><h2 style={{ margin: 0 }}>{product ? "✏️ Editar producto" : "➕ Agregar producto"}</h2><p className="muted">Misma estructura de datos de la app de escritorio.</p></div><button className="btn btn-secondary" onClick={onClose}>Cerrar</button></div>
    <form onSubmit={save}>
